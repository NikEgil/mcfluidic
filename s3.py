...
def get_max_rmr(path):
    ...
    for j in range(10000):
        spec = open(path[j], "r", encoding="utf8")
        s = spec.read()
        spec.close()
    xt = time.time() - st
...
script_path = r"C:\Users\Nik\Desktop\prog\перед 2"
...
for i in range(len(file_list)):
    pu.append(script_path + "/" + str(file_list[i]))
print(pu[0:10])
data_max = get_max_rmr(pu)
